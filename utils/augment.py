    ...
    LEVEL_TO_ARG.update(
        {
            ...
            "TranslateXRel": partial(
                _level_to_arg, key="translate_x_pct", default=0.45
            ),
            "TranslateYRel": partial(
                _level_to_arg, key="translate_y_pct", default=0.45
            ),
        }
    )
...
def rand_augment_transform(magnitude=5, num_layers=3):
    ...
    ra_ops = auto_augment.rand_augment_ops(
        magnitude, hparams, transforms=_RAND_TRANSFORMS
    )
